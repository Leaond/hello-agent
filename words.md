...
| 65   | Feedforward Neural Network Language Model | /ˈfiːdfɔːrwərd ˈnjʊərəl ˈnetwɜːrk ˈlæŋɡwɪdʒ ˈmɒdəl/ | n.   | 前馈神经网络语言模型     | Bengio 提出的早期神经语言模型，用前馈网络根据固定窗口的前文词预测下一个词，并同时学习词向量表示      |
| 66   | Negative Sampling                         | /ˈneɡətɪv ˈsɑːmplɪŋ/                                | n.   | 负采样                   | 词向量训练中的加速技巧，只采样少量负样本代替全词表 Softmax 计算，大幅降低训练开销                    |
| 67   | Hierarchical Softmax                      | /ˌhaɪəˈrɑːrkɪkəl ˈsɒftmæks/                         | n.   | 分层 Softmax             | 把词表组织为哈夫曼树，将一次多分类拆解为路径上的多次二分类，把复杂度从 O(V) 降到 O(log V)            |
| 68   | Projection Layer                          | /prəˈdʒekʃən ˈleɪər/                                | n.   | 投影层                   | 将离散的词或高维稀疏输入映射为低维稠密向量的网络层，是词嵌入查表操作的实现形式                       |
| 69   | Gating Mechanism                          | /ˈɡeɪtɪŋ ˈmekənɪzəm/                                | n.   | 门控机制                 | 通过可学习的门控单元控制信息保留与丢弃比例的结构，是 LSTM、GRU 缓解梯度消失的关键设计                |
| 70   | Long Short-Term Memory (LSTM)             | /lɒŋ ʃɔːrt tɜːrm ˈmeməri/                           | n.   | 长短时记忆网络           | 引入输入门、遗忘门和输出门及细胞状态的循环神经网络变体，能有效捕获长距离依赖                         |
| 71   | Scaled Dot-Product Attention              | /skeɪld dɒt ˈprɒdʌkt əˈtenʃən/                      | n.   | 缩放点积注意力           | Transformer 的注意力计算方式，将查询与键的点积除以维度平方根后过 Softmax，避免梯度过小               |
| 72   | Autoregressive                            | /ˌɔːtoʊrɪˈɡresɪv/                                   | adj. | 自回归的                 | 逐个位置依次生成、每步都以已生成内容为条件的建模方式，是 GPT 类模型的生成范式                        |
| 73   | Masked Self-Attention                     | /mɑːskt self əˈtenʃən/                              | n.   | 掩码自注意力             | 在自注意力中屏蔽当前位置之后的所有位置，确保生成时不能看到未来信息，保证自回归性质                   |
| 74   | Zero-shot Prompting                       | /ˈzɪəroʊ ʃɒt ˈprɒmptɪŋ/                             | n.   | 零样本提示               | 不提供任何示例，仅用自然语言指令让模型直接完成任务的提示方式                                         |
| 75   | One-shot Prompting                        | /wʌn ʃɒt ˈprɒmptɪŋ/                                 | n.   | 单样本提示               | 在提示词中给出一个示例，帮助模型理解任务格式和期望输出的提示方式                                     |
| 76   | Few-shot Prompting                        | /fjuː ʃɒt ˈprɒmptɪŋ/                                | n.   | 少样本提示               | 在提示词中给出少量示例引导模型完成任务，依赖上下文学习能力，无需更新模型参数                         |
| 77   | Instruction Tuning                        | /ɪnˈstrʌkʃən ˈtjuːnɪŋ/                              | n.   | 指令调优                 | 用大量「指令—回答」数据对预训练模型微调，使其学会理解并遵循人类指令                                  |
| 78   | Tokenizer                                 | /ˈtoʊkənaɪzər/                                      | n.   | 分词器                   | 执行分词的组件，负责在文本与词元 ID 序列之间双向转换，是模型输入输出的必要环节                       |
| 79   | Token                                     | /ˈtoʊkən/                                           | n.   | 词元                     | 模型处理文本的最小单位，可以是词、子词或字符，也是计费和上下文长度的计量单位                         |
| 80   | Out-Of-Vocabulary (OOV)                   | /aʊt əv vəˈkæbjəleri/                               | n.   | 未登录词                 | 未出现在模型词表中的词，传统分词会退化为未知标记，子词分词可有效缓解该问题                           |
| 81   | Subword Tokenization                      | /ˈsʌbwɜːrd ˌtoʊkənaɪˈzeɪʃən/                        | n.   | 子词分词                 | 把词进一步切分为更小的高频片段的分词策略，兼顾词表规模与未登录词覆盖能力                             |
| 82   | Byte-Pair Encoding (BPE)                  | /baɪt peər ɪnˈkoʊdɪŋ/                               | n.   | 字节对编码               | 从字符出发反复合并语料中最高频相邻单元来构建子词词表的算法，被 GPT 系列广泛采用                      |
| 83   | Nucleus Sampling (Top-p)                  | /ˈnjuːkliəs ˈsɑːmplɪŋ/                              | n.   | 核采样                   | 只在累积概率达到阈值 p 的最小候选词集合内采样的解码策略，兼顾生成多样性与合理性                      |
