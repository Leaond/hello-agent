...
#         1.初始执行
        ...
        initial_result = self.get_llm_response(initial_prompt,**kwargs)
    ...
    #         a. 反思
            ...
            feedback = self.get_llm_response(reflect_prompt,**kwargs)
    ...
    #         b.检查是否需要停止
            ...
            # c.优化
            print("\n-> 正在进行优化...")
            refined_result = self.prompts['refine'].format(task=input_text,last_attempt=last_result,feedback=feedback)
            feedback = self.get_llm_response(refined_result,**kwargs)
    ...
    def get_llm_response(self,prompt:str,**kwargs) ->str:
        ...
        return self.llm.invoke(message,**kwargs) or ""
